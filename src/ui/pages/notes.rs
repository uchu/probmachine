...
fn render_piano_container(ui: &mut egui::Ui) {
    ...
    egui::Frame::default()
        ...
        .corner_radius(15.0)
        .show(ui, |ui| {
            render_piano_keys(ui, container_height);
        });
}

fn render_piano_keys(ui: &mut egui::Ui, container_height: f32) {
    let container_rect = ui.available_rect_before_wrap();

    let padding = 10.0;
    let keyboard_width = container_rect.width() - (padding * 2.0);
    let keyboard_height = container_height - (padding * 2.0);
    ...
    for _octave in 0..2 {
        for &_note in &white_key_pattern {
            let x = container_rect.min.x + padding + (white_key_index as f32 * white_key_width);
            let y = container_rect.min.y + padding;
    ...
    for _octave in 0..2 {
        for i in 0..white_key_pattern.len() {
            ...
            if has_black_key_after {
                let x = container_rect.min.x + padding + (white_key_index as f32 * white_key_width) + white_key_width - (black_key_width / 2.0);
                let y = container_rect.min.y + padding;
    ...
    ui.allocate_space(egui::vec2(container_rect.width(), container_height));
}
